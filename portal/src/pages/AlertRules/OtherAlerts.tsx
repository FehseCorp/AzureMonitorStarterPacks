...
import { ARG_OTHER_ALERTS } from "../../services/queries/argQueries";
...
export function OtherAlerts() {
  ...
  const alertsQ = useARGQuery(["otherAlerts"], ARG_OTHER_ALERTS);
  ...
  const action = useBackendAction({
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["otherAlerts"] });
    },
  });
  ...
  if (alertsQ.isLoading) {
    return <Spinner size="medium" label="Loading other alerts..." />;
  }
  ...
  return (
    <div className={styles.container}>
      <Title3>Other Alert Rules</Title3>
      ...
      {pagedRows.length > 0 ? (
        ...
        <Text>No other alert rules found.</Text>
      )}
